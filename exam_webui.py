    ...
    try:
        ...
        try:
            ...
            if backup_files:
                ...
                create_sample_md_file()  # 如果没有备份，创建示例文件
...
if __name__ == '__main__':
    # 初始化目录
    for directory in [app.config['SESSION_FILE_DIR'], app.config['UPLOAD_FOLDER']]:
        if not os.path.exists(directory):
            os.makedirs(directory)
            app.logger.info(f"创建目录: {directory}")
    
    # 启动应用（Docker环境中不使用SSL）
    try:
        app.logger.info("=== 启动测试系统（Docker版本）===")
