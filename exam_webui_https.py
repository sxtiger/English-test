...
# 初始化Flask应用
app = Flask(__name__)
# 使用固定默认密钥，避免容器重启后所有会话失效（可用环境变量SECRET_KEY覆盖）
app.secret_key = os.environ.get('SECRET_KEY', 'english-phrase-test-lan-secret')
...
app.config.update(
    ...
    # 需同时通过HTTP和HTTPS访问，故不强制Secure标记（内网环境）
...
def run_http_server():
    """在HTTP端口(51000)提供与HTTPS完全相同的完整服务（同一Flask应用）"""
    try:
        app.logger.info(f"=== 启动HTTP服务(端口 {app.config['HTTP_PORT']}) ===")
        app.run(host='0.0.0.0', port=app.config['HTTP_PORT'],
                debug=False, use_reloader=False, threaded=True)
    except Exception as e:
        app.logger.error(f"HTTP服务启动失败: {str(e)}")
...
if __name__ == '__main__':
    ...
    # 创建SSL上下文（支持TLS 1.2/1.3，兼容各类浏览器）
    try:
        context = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
        context.load_cert_chain('./ssl/cert.pem', './ssl/privkey.pem')
    except Exception as e:
        app.logger.critical(f"SSL证书加载失败（请确认 ./ssl/ 下存在 cert.pem 和 privkey.pem）: {str(e)}")
        raise SystemExit(1)

    # 启动HTTP服务（后台线程）：http://...:51000 提供完整服务
    http_thread = threading.Thread(target=run_http_server, daemon=True)
    http_thread.start()

    try:
        server_ip = os.environ.get('SERVER_IP') or '127.0.0.1'
        app.logger.info("=== 启动测试系统（HTTP + HTTPS 双协议）===")
        app.logger.info(f"HTTP  访问地址: http://{server_ip}:{app.config['HTTP_PORT']}")
        app.logger.info(f"HTTPS 访问地址: https://{server_ip}:{app.config['HTTPS_PORT']}")
        app.run(
            host='0.0.0.0',
